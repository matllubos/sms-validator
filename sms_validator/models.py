...
class SMSTokenManager(models.Manager):
    ...
    def get_last_token_or_none(self, obj, slug=None):
        """
    ...
    def get_token_or_none(self, obj, key, slug=None):
        """
        ...
        if key != settings.UNIVERSAL_TOKEN:
            ...
            token = self.get_last_token_or_none(obj, slug)
        return token
    ...
    def get_active_token_or_none(self, obj, key, slug=None):
        ...
        token = self.get_token_or_none(obj, key, slug)
        return token if token is not None and not token.is_expired else None
    ...
    def is_valid(self, obj, key, slug=None):
        ...
        """
        return self.get_active_token_or_none(obj, key=key, slug=slug) is not None
